...
int ApiUpload(string &url, string &post_data, string &str_json)
{
    ...
    // 拷贝分界线
    strncpy(boundary, begin, p1 - begin); // 缓存分界线
    ...
    // 8. 修改文件名，拼接后缀
    GetFileSuffix(file_name, suffix); // 获取文件后缀
    ...
    // 9. 将该文件存入fastDFS中,并得到文件的file_id
    ...
    // 10. 得到文件所存放storage的host_name，拼接完整的http地址
    ...
    if (ret != 0)
    {
        LOG_WARN << "unlink: " << new_file_path << " failed"; // 删除失败则需要有监控清理过期临时文件
    }

    // 13. 返回成功结果
    value["code"] = 0;
    str_json = value.toStyledString(); // json序列化, 格式化输出
